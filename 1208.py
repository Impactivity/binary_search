...
# _array = list(map(int, sys.stdin.readline().split()))

#
#
# def binary_search(start,end,t, target):
#
#     if start > end :
#         return start
#
#     mid = (start + end) // 2
#
#     if array[t][mid] < target:
#         return binary_search(mid+1 , end, t, target )
#     elif array[t][mid] == target:
#         return mid
#     else :
#         return binary_search(start, mid - 1 ,t, target)
#
#
#
# cnt = 0
#
#
# for i in range(1,len(_array)+1):
#     # print('현재 i 값 : ', i)
#     array = list(map(list,combinations(_array, i)))
#     print(array)
#     # print(array)
#     for k in range(0, len(array)):
#         # print('현재 k 값 :', k)
#         total = [array[k][0]]
#         for j in range( 1 , len(array[k])):
#             if array[k][j] > total[-1]:
#                 total.append(array[k][j])
#             else :
#                 total[binary_search(0,len(total) - 1 ,k, array[k][j])] = array[k][j]
#         if sum(total) == S:
#             print(total)
#             cnt += 1
#
